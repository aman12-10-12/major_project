...
const getWeb3 = async () => {
  ...
  if (window.ethereum) {
    ...
    try {
      // Request account access if needed (modern method)
      await window.ethereum.request({ method: 'eth_requestAccounts' });
      console.log("Web3 initialized successfully", tempWeb3);
  ...
  // Legacy dapp browsers...
    ...
    console.log("Using legacy web3 provider", tempWeb3);
...
const App = () => {
  ...
  const init = async () => {
    try {
      ...
      // Check if web3 was successfully initialized
      if (!tempWeb3) {
      ...
      if (networkdata) {
        ...
        console.error("Smart contract not deployed to detected network. Please check your network in MetaMask.");
      }
    } catch (error) {
