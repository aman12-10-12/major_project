...
function Index() {
  ...
  const uploadFile = async (file) => {
    try {
      ...
      if (!apiKey) {
        console.error("Lighthouse API key not found. Please add REACT_APP_LIGHTHOUSE_API_KEY to your .env file");
        toast.error("Lighthouse API key missing. Check console for details.");
        return;
      }
      ...
      toast.error("Failed to upload file. Check your Lighthouse API key.");
    }
  };
  ...
  const uploadTipDataToLightHouse = async (tip) => {
    try {
      ...
      console.log("Tip data uploaded to Lighthouse:", response.data.Hash);
      return response.data.Hash;
    } catch (error) {
      console.error("Error uploading tip data to Lighthouse:", error);
      throw error;
    }
  };
